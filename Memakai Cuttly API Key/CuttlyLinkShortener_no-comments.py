...
TOKEN = "Masukkan token API Cuttly akun anda disini"   

def shorten_link(link):
    payload = {'key': TOKEN, 'short': link}
    response = requests.get('https://cutt.ly/api/api.php', params=payload)
    data = response.json()
    if data['url']['status'] == 7:
        return data['url']['shortLink']
    else:
        raise Exception('Link cannot be shortened!')
...
window = tk.Tk()
window.title("Cuttly Link Shortener")
